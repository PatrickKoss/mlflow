...
Result: **17/17 surfaces passed; 0 failed.** Each row was rendered in headless Chromium from the real production React build. Browser page/console errors and unexpected failed same-origin responses fail the owning test, and every same-origin response is audited for zero `X-MLflow-Backend: python`.
...
| Gateway endpoint guardrails | `#/gateway/endpoints/e-44e735bb1f6d44c9abef4cd9e0e2d572?tab=guardrails` | Endpoint detail Guardrails tab and seeded guardrail rendered; zero Python-attributed responses. | **PASS** | `gateway-guardrails.png` | Guardrail was attached through Rust RPCs; no model invocation occurred. |
...
| Demo seeded custom view | `#/experiments/2/prompts/t22-5-support-prompt?promptVersion=2&selectedEvaluationId=tr-11111111111111111111111111111111` | The demo's Span review custom view rendered for a seeded trace. | **PASS** | `demo-custom-view.png` | The view definition came from POST /mlflow/demo/generate. |
| Issues | `#/experiments/1/evaluation-runs/d47800627217445286eef8c067e6e4ca/issues` | Issue-detection run Issues tab and seeded run-linked issue rendered; zero Python-attributed responses. | **PASS** | `issues.png` | Populated pending issue state. |
...
| Dataset records | `#/experiments/1/datasets/d-f71b68882ee44683a785901b64269dfb` | Dataset detail and populated records table rendered; zero Python-attributed responses. | **PASS** | `dataset-records.png` | Two deterministic records, one linked to a seeded trace. |
...
| Assistant panel (compose unauthenticated state) | `#/experiments/1/evaluation-runs` | Assistant panel auto-opens with contextual prompt suggestions and the popular-provider quick select (local CLI providers disabled, Ollama/gateway available); remote message send rejected with the localhost-only detail; zero Python-attributed responses, including assistant config. | **PASS** | `assistant.png` | The expected config response was Rust-attributed 403. Authenticated CLI chat frame parity is covered by rust/compliance/recorders/test_assistant_cli_provider_differential.py (T20.2). |
<!-- prettier-ignore-end -->
