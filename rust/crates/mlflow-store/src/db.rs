...
pub const EXPECTED_ALEMBIC_HEAD: &str = "b7e2c1a4d9f3";
                ...
                let pool = mysql_pool_options(&cfg)
                    .after_connect(|conn, _meta| {
                        Box::pin(async move {
                            ...
                            // NO_AUTO_VALUE_ON_ZERO permits the idempotent raw
                            // insert of the required default experiment ID 0.
...
mod tests {
    ...
    fn expected_head_is_current_alembic_head() {
        ...
        assert_eq!(EXPECTED_ALEMBIC_HEAD, "b7e2c1a4d9f3");
    }
