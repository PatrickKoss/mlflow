...
const PORTKEY_CONFIG: &str = r#"{"auth_modes":[{"config_fields":[{"description":"Provider to route to: a Model Catalog slug (e.g. @openai-prod) or a bare provider slug (e.g. openai)","name":"portkey_provider","required":false,"type":"string"},{"description":"Portkey API Base URL (defaults to https://api.portkey.ai/v1)","name":"api_base","required":false,"type":"string"}],"description":"Use Portkey API Key with an optional routing target","display_name":"API Key","mode":"api_key","secret_fields":[{"description":"Portkey API Key","name":"api_key","required":true,"type":"string"},{"description":"Portkey config ID (e.g. pc-xxxx) or JSON config. Stored as a secret since a raw JSON config may embed upstream credentials","name":"portkey_config","required":false,"type":"string"},{"description":"Upstream provider API key (only for bare provider slugs not stored in Portkey)","name":"provider_api_key","required":false,"type":"string"}]}],"default_mode":"api_key"}"#;
const SAP_AI_CORE_CONFIG: &str = r#"{"auth_modes":[{"config_fields":[],"description":"Authentication is injected by the configured egress gateway","display_name":"Egress Gateway","mode":"default_chain","secret_fields":[]}],"default_mode":"default_chain"}"#;
...
pub async fn provider_config(_workspace: Workspace, parts: Parts) -> Result<Response, MlflowError> {
    ...
    let raw = match normalized.as_str() {
        ...
        "portkey" => Some(PORTKEY_CONFIG),
        "sap-ai-core" => Some(SAP_AI_CORE_CONFIG),
        _ => None,
    };
